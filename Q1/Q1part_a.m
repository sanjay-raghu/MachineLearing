...
u(j,:)= mean(M1);
sig(j,1) = {cov(M1)+.7*eye(1024)};%added regulerisation term and lambda = .7
...
for j=1:3
    ...
    for i=1:100
        ...
        P1 = -((y-u(1,:))*(inv(sigma(1:1024,1:1024)))*(y-u(1,:))')-0.5*log(det(sigma(1:1024,1:1024)))-log(3);
        P2 = -((y-u(2,:))*(inv(sigma(1025:2048,1:1024)))*(y-u(2,:))')-0.5*log(det(sigma(1025:2048,1:1024)))-log(3);
        P3 = -((y-u(3,:))*(inv(sigma(2049:3072,1:1024)))*(y-u(3,:))')-0.5*log(det(sigma(2049:3072,1:1024)))-log(3);
...
count
miss_classified
save('acuracy_part_a.mat','count','miss_classified');
